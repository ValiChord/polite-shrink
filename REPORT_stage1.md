...
Holochain's DHT layer (kitsune2) supports per-agent *storage arcs* — the contiguous range of DHT locations an agent claims authority over — but has run all agents at full arcs since dynamic arc sizing was disabled by default following its 2021 test campaign. That campaign documented control-loop oscillation, which Holochain called ["the hallway dance"](https://blog.holochain.org/testing-sharding/) (Dev Pulse 107, 13 Nov 2021) — but never what the oscillation cost in durability. This study measures that, and the answer is sharper than "oscillation loses data": **damping the oscillation does not stop the loss.** Kitsune2 issue #160 asks for the missing *sharding module*: a controller that recommends a target arc from observed redundancy.

*Two scoping notes, since this study is otherwise easy to over-read as history. Holochain's stated reason for disabling sharding was that the feature was not as well-tested as they wanted, and that switching it off reduced the surface they had to test and maintain while reaching stability — not observed data loss. And the naive controller below is our reconstruction of an undamped rule, not a port of whatever Holochain ran in 2021; the loss rates are properties of the rules we model.*
...
The naive controller reproduces the oscillation Holochain documented in 2021, and additionally loses data in 95.9% of runs. Damping the oscillation does not stop that: hysteresis still leaves a ~24% loss rate, and decision-epoch jitter — the textbook desynchronisation remedy — provides no additional safety. The durability risk is therefore *not* the symptom the oscillation makes visible. A **two-phase "polite shrink"** (announce intent → wait a multiple of measured staleness → re-check with a deterministic lowest-id tie-break) lost **zero sectors in all 1,248 runs** (95% upper bound on the per-run loss rate: 0.24%) and could not be defeated by the adversarial search. Porting the controller into the real codebase surfaced three constraints invisible to the simulation, most notably that **death visibility and view staleness run on independent clocks**, which bounds the safety of any locally-decided shrink. The port, a proposed upstream bug fix (PR #572, open), and all findings were offered to the kitsune2 issue tracker.
...
1. **V0 reproduces the documented historical symptom** — persistent oscillation — the one point of contact between this model and the 2021 experience, and so the only sense in which the model is validated against it. It also shows what that write-up never measured: the undamped rule doesn't merely oscillate, it loses data in 95.9% of runs. (A property of the rule we model — V0 is a reconstruction, not a port of Holochain's 2021 code.)
2. **Hysteresis does most of the stabilising** (95.9% → 24.0%) but cannot close the shrink race: agents acting on stale views still occasionally abandon the same sectors simultaneously. **This dissociates the oscillation from the data loss** — V1 largely removes the hallway dance and still loses data in a quarter of runs. The two are separate consequences of the same root (deciding on stale views), and the durability risk is not the one the oscillation makes visible: what destroys data is the simultaneous unilateral drop, which damping leaves wide open.
...
Mitigation implemented: a **storm brake** — whenever any peer newly becomes invisible, all pending shrink intents on the node are cancelled and their dwell counters reset. A residual window remains for deaths younger than the detection latency at execute time; no locally-decided rule can close it, only bound it. Design implication for #160: the shrink wait must be configured to dominate *death-detection latency*, not merely gossip staleness.

> **Later work on this window (2026-08-07).** "Only bound it" still holds — the residual is never driven to zero — but how *tightly* it can be bounded locally, and by what, has since been measured. A node cannot infer the window from its own observation history ([REPORT_mz_decomposition.md](REPORT_mz_decomposition.md): the failure mode grows 29× with detection latency while the recoverable-from-history term stays flat). It *can* be offset by biasing the detector toward convicting the living, which cuts `P(any loss)` from 40% to 2% in the slow-detection stress regime — by cancelling the over-count, not by inferring it. See [REPORT_stage3.md](REPORT_stage3.md) §11 amendment and constraint 6b. We also note the failure is one of *declared* coverage: the shrinking node still holds the op data on disk, so prompt re-grow recovers without data loss provided garbage collection is deferred accordingly.
