...
1. **V0 reproduces the documented historical symptom** — persistent oscillation — which is the one point of contact between this model and the 2021 experience, and so the only sense in which the model is validated against it. The near-certain data loss V0 also exhibits is a property of the modelled rule; Holochain's 2021 write-up reports oscillation, not loss.
